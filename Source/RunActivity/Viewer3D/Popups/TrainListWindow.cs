...
namespace Orts.Viewer3D.Popups
{
 ...
 //                       && ! (thisTrain.TrainType == Train.TRAINTYPE.AI_INCORPORATED && !thisTrain.IncorporatingTrain.IsPathless))
                    && !(thisTrain.TrainType == Train.TRAINTYPE.AI_INCORPORATED && !thisTrain.IncorporatingTrain.IsPathless) && !thisTrain.Autopilot) //joe179star autopilot
                    {
                        var line = scrollbox.AddLayoutHorizontalLineOfText();
                        TrainLabel number, name, viewed;
...
//                Viewer.Simulator.IsAutopilotMode && PickedTrainFromList.IsPlayable)
                Viewer.Simulator.IsAutopilotMode && PickedTrainFromList.IsPlayable && (Viewer.PlayerTrain as AITrain).MovementState != AITrain.AI_MOVEMENT_STATE.AI_STATIC) //joe179star autopilot
            {
                //               if (UserInput.IsDown(UserCommand.GameSuspendOldPlayer))
                if (UserInput.IsDown(UserCommand.GameSuspendOldPlayer) && !Viewer.Simulator.TimetableMode) //joe179star autopilot
