...
using Orts.Simulation.Timetables; //joe179star autopilot
...
namespace Orts.Viewer3D
{
 ...
 //                   || !IsReverserInNeutral(PlayerLocomotive))
                      || !IsReverserInNeutral(PlayerLocomotive) && !((PlayerLocomotive.Train.Autopilot) || PlayerLocomotive.Train.TrainType == Train.TRAINTYPE.AI_PLAYERHOSTING)) //joe179star autopilot
                {
            ...
            if (UserInput.IsPressed(UserCommand.GameAutopilotMode))
            {
                ...
                if (playerTrain.TrainType == Train.TRAINTYPE.AI_PLAYERHOSTING || playerTrain is TTTrain && playerTrain.Autopilot) //joe179star autopilot
                {
//                    var success = ((AITrain)PlayerLocomotive.Train).SwitchToPlayerControl();
                    var success = ((AITrain)playerTrain).SwitchToPlayerControl(); //joe179star autopilot
                    if (success)
                    {   
                ...
                else if (playerTrain.TrainType == Train.TRAINTYPE.AI_PLAYERDRIVEN || playerTrain is TTTrain && !playerTrain.Autopilot)  //joe179star autopilot
                {
                    //                   if (PlayerLocomotive.Train.ControlMode == Train.TRAIN_CONTROL.MANUAL)
                    if (playerTrain.ControlMode == Train.TRAIN_CONTROL.MANUAL) //joe179star autopilot
                    ...
                    else
                    {
                        ...
                        var success = ((AITrain)playerTrain).SwitchToAutopilotControl(); //joe179star autopilot
                        if (success)
                        {
        ...
        public void ChangeCab()
        {
            ...
            if (!Simulator.PlayerLocomotive.HasFront3DCab && !Simulator.PlayerLocomotive.HasRear3DCab &&
               (Simulator.PlayerLocomotive.HasFrontCab || Simulator.PlayerLocomotive.HasRearCab)) //joe179star autopilot
            ...
            ThreeDimCabCamera.Activate(); //joe179star autopilot
            SetCommandReceivers();
            if (MPManager.IsMultiPlayer())
